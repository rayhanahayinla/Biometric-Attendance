            ...
            if (empty($row['username'])) {
                ...
                if (!empty($Uname) && !empty($Number) && !empty($Email)) {
                    ...
                    if (!mysqli_stmt_prepare($result, $sql)) {
                        ...
                        $resultl = mysqli_stmt_get_result($result);
                        if (!$row = mysqli_fetch_assoc($resultl)) {
                            $sql="UPDATE users SET username=?, matricnumber=?, gender=?, email=?, user_date=CURDATE(), time_in=? WHERE fingerprint_select=1";
                            $result = mysqli_stmt_init($conn);
                            if (!mysqli_stmt_prepare($result, $sql)) {
                                echo "SQL_Error_select_Fingerprint";
                                exit();
                    ...
                    if (!mysqli_stmt_prepare($result, $sql)) {
                        ...
                        if (!$row = mysqli_fetch_assoc($resultl)) {
                            ...
                            if (!empty($Uname) && !empty($Email) && !empty($Timein)) {
                                ...
                                $sql="UPDATE users SET username=?, matricnumber=?, gender=?, email=?, time_in=? WHERE fingerprint_select=1";
                                $result = mysqli_stmt_init($conn);
                                if (!mysqli_stmt_prepare($result, $sql)) {
                                    echo "SQL_Error_select_Fingerprint";
                                    exit();
...
if (isset($_POST['delete'])) {
    ...
    if (!mysqli_stmt_prepare($result, $sql)) {
        ...
        $resultl = mysqli_stmt_get_result($result);
        if ($row = mysqli_fetch_assoc($resultl)) {
            $sql="UPDATE users SET username='', matricnumber='', gender='', email='', time_in='', del_fingerid=1 WHERE fingerprint_select=1";
            $result = mysqli_stmt_init($conn);
            if (!mysqli_stmt_prepare($result, $sql)) {
